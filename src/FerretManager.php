...
use Cline\Ferret\Exceptions\ArrayValueCannotBeNullException;
use Cline\Ferret\Exceptions\ArrayValueCannotBeScalarException;
use Cline\Ferret\Exceptions\BooleanValueCannotBeArrayException;
use Cline\Ferret\Exceptions\BooleanValueCannotBeNullException;
...
use Cline\Ferret\Exceptions\FloatValueCannotBeArrayException;
use Cline\Ferret\Exceptions\FloatValueCannotBeNullException;
use Cline\Ferret\Exceptions\FloatValueMustBeNumericException;
use Cline\Ferret\Exceptions\IntegerValueCannotBeArrayException;
use Cline\Ferret\Exceptions\IntegerValueCannotBeNullException;
use Cline\Ferret\Exceptions\IntegerValueMustBeNumericException;
use Cline\Ferret\Exceptions\InvalidArrayValueException;
use Cline\Ferret\Exceptions\InvalidBase64KeyException;
use Cline\Ferret\Exceptions\InvalidBooleanValueException;
use Cline\Ferret\Exceptions\InvalidConfigurationException;
use Cline\Ferret\Exceptions\InvalidFloatValueException;
use Cline\Ferret\Exceptions\InvalidIntegerValueException;
use Cline\Ferret\Exceptions\InvalidStringValueException;
...
use Cline\Ferret\Exceptions\StringValueCannotBeArrayException;
use Cline\Ferret\Exceptions\StringValueCannotBeNullException;
 ...
 *
 * Provides a comprehensive fluent API for searching, loading, reading, writing,
 * encrypting, and transforming configuration files in multiple formats (JSON,
 * YAML, NEON, INI, XML, PHP). The manager handles configuration caching, type-safe
 * value access, environment variable interpolation, and configuration diffing.
 *
 * Key features:
 * - Multi-format support with pluggable loaders
 * - Intelligent configuration file searching with multiple strategies
 * - Type-safe accessor methods (string(), integer(), float(), boolean(), array())
 * - Encryption and decryption for sensitive configuration data
...
final class FerretManager
{
     ...
     *
     * Stores the original SearchResult objects returned from configuration
     * loading operations. These represent the last-saved state of each module
     * and are used for dirty tracking and rollback operations.
     *
     ...
     *
     * Tracks the current working state of configuration data for each module.
     * Changes made through set(), push(), prepend(), and forget() methods
     * modify this array. Differences between this and $configurations indicate
     * unsaved changes that can be persisted with save() or discarded with rollback().
     *
     ...
     *
     * Caches Searcher instances created via explorer() method for reuse across
     * multiple operations. Each searcher is configured with module-specific options
     * and handles the logic for locating configuration files in the filesystem.
     *
     ...
     *     ignoreEmpty?: bool
     * } $defaultOptions Default configuration options applied to all new searchers
     *                    created by this manager. Individual explorer() calls can
     *                    override these defaults with their own options. Includes
     *                    settings for search paths, file loaders, caching behavior,
     *                    search strategy (None, Global, or Local), directory
     *                    boundaries, transformation functions, and empty file handling.
     */
     ...
     *
     * @throws InvalidStringValueException If the value cannot be cast to string
     *
    ...
    public function string(string $moduleName, string $key): string
    {
        ...
        if ($value === null) {
            throw StringValueCannotBeNullException::forKey($moduleName, $key);
        }

        if (is_array($value)) {
            throw StringValueCannotBeArrayException::forKey($moduleName, $key);
        }
     ...
     * @param string $key        The key to retrieve (dot notation)
     *
     * @throws InvalidIntegerValueException If the value cannot be cast to integer
     *
     * @return int The configuration value as an integer
     */
    public function integer(string $moduleName, string $key): int
    {
        $value = $this->get($moduleName, $key);

        if ($value === null) {
            throw IntegerValueCannotBeNullException::forKey($moduleName, $key);
        }

        if (is_array($value)) {
            throw IntegerValueCannotBeArrayException::forKey($moduleName, $key);
        }

        if (!is_numeric($value)) {
            throw IntegerValueMustBeNumericException::forKey($moduleName, $key);
        }
     ...
     * @param string $key        The key to retrieve (dot notation)
     *
     * @throws InvalidFloatValueException If the value cannot be cast to float
     *
     * @return float The configuration value as a float
     */
    public function float(string $moduleName, string $key): float
    {
        $value = $this->get($moduleName, $key);

        if ($value === null) {
            throw FloatValueCannotBeNullException::forKey($moduleName, $key);
        }

        if (is_array($value)) {
            throw FloatValueCannotBeArrayException::forKey($moduleName, $key);
        }

        if (!is_numeric($value)) {
            throw FloatValueMustBeNumericException::forKey($moduleName, $key);
        }
     ...
     * @param string $key        The key to retrieve (dot notation)
     *
     * @throws InvalidBooleanValueException If the value cannot be cast to boolean
     *
     * @return bool The configuration value as a boolean
     */
    public function boolean(string $moduleName, string $key): bool
    {
        $value = $this->get($moduleName, $key);

        if ($value === null) {
            throw BooleanValueCannotBeNullException::forKey($moduleName, $key);
        }

        if (is_array($value)) {
            throw BooleanValueCannotBeArrayException::forKey($moduleName, $key);
        }
     ...
     * @param string $key        The key to retrieve (dot notation)
     *
     * @throws InvalidArrayValueException If the value is not an array
     *
     * @return array<array-key, mixed> The configuration value as an array
     */
    public function array(string $moduleName, string $key): array
    {
        $value = $this->get($moduleName, $key);

        if ($value === null) {
            throw ArrayValueCannotBeNullException::forKey($moduleName, $key);
        }

        if (!is_array($value)) {
            throw ArrayValueCannotBeScalarException::forKey($moduleName, $key);
        }
    ...
    /**
     * Get a configuration value as a Collection.
     *
     * @param string $moduleName The module name
     * @param string $key        The key to retrieve (dot notation)
     *
     * @throws InvalidArrayValueException If the value is not an array
     *
     * @return Collection<array-key, mixed> The configuration value as a Collection
     */
    public function collection(string $moduleName, string $key): Collection
    {
        return new Collection($this->array($moduleName, $key));
    }
     ...
     * @param null|string $glob      Glob pattern to filter files (e.g., '*.json')
     *
    ...
    private function collectFilesForEncryption(string $directory, bool $recursive, ?string $glob): array
    {
        ...
        if ($matches !== false) {
            foreach ($matches as $match) {
                ...
                if (is_file($match) && !str_ends_with($match, '.encrypted')) {
                    $files[] = $match;
                }
            }
        }
